    ...
    public class MovieService : IMovieService
    {
        ...
        public async Task<List<MovieSearchView>> SearchMovieItems(string searchTerm)
        {
            ...
            if (moviesResultDb.Count() > 10)
            {
                ...
                return moviesResultDb.Select(m => new MovieSearchView(m, _context)).ToList();
            }
            // If  we don't have enough results from DB, we fetch them from the external API
            var movieSearchResults = await GetMoviesFromExt(searchTerm);
            // Converting the results to the view model and sending them to the client
        ...
        /// <param name="searchTerm"></param>
        /// <returns></returns>
        private async Task<List<MovieResult>> GetMoviesFromExt(string searchTerm)
        {
            ...
            return JsonConvert.DeserializeObject<MovieSearchResults>(jsonResponse).Results;
        }
    }
}